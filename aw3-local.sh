...
function installAw2(){
	...
	wp config set AWESOME_PATH /Users/macbook/development/awesome-enterprise --add=true --type=constant --allow-root
    ...
    chown -R www-data:www-data wp-content/uploads/*
     ...
     wp config set CONNECTIONS "array('awesome_cdn'=> array('connection_service'=>'url_conn', 'url'=>'https://cdn.getawesomestudio.com/code', 'redis_db'=>1, 'cache_expiry'=>1440))" --raw --add --type=constant --allow-root
...
function baseImport(){
	if [ -z "$1" ]; then
		echo 'Usage: ./aw3-local.sh --base-import <example.com>'
		exit
	fi	
	echo 'Checking site...'$1
	if [ ! -d "/Users/MacBook/development/$1" ]; then
		echo "Site does not exists"
		exit
	else
		cd /Users/MacBook/development//$1
...
function createUser(){
	if [ -z "$1" ]; then
		echo 'Usage: ./aw3-local.sh --create-user <example.com>'
		exit
	fi	
	 echo 'Checking site...'
...
function setup(){
	if [ -z "$1" ]; then
		echo 'Usage: ./aw3-local.sh --setup <example.com>'
		exit
	fi	
	
	echo 'Checking site...'
	if [ -d "/Users/MacBook/development/$1" ] 
	then
		cd /Users/MacBook/development/$1
		installAw2 $1
		createWpUserPrompt $1
...
function updateAw3(){
	...
	printf "${GREEN} ...NOT ALLOWED on LOCALHOST... \n${NC}"
	printf "${ORANGE} ...[✌]... \n${NC}"
}
...
{
    cat <<-EOF
  Usage: aw3-local.sh [options] <site>
  Options:
